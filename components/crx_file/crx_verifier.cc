...
// The SHA256 hash of the Jatter Omaha Update key.
constexpr KeyHash kJatterPublisherKeyHash = {
    0x0d, 0xab, 0xa0, 0x9e, 0xd9, 0x35, 0x0e, 0xed, 0xac, 0x9a, 0x4f,
    0x64, 0x8c, 0x4f, 0x0f, 0x68, 0x10, 0x74, 0xf5, 0xda, 0xcf, 0x9c,
    0xc9, 0xd4, 0x70, 0x34, 0x7f, 0xac, 0x17, 0xdd, 0xdd, 0x46};
  ...
  for (const auto& proof_type : proof_types) {
    for (const auto& proof : (header.*proof_type.first)()) {
      ...
      found_publisher_key =
          ...
          key_hash == kJatterPublisherKeyHash ||
